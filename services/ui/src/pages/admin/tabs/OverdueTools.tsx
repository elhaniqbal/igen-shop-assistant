...
function OverdueActions({ loan, onDone }: { loan: LoanOut; onDone: () => void }) {
  ...
  const extend = async () => { setBusy(true); setNote(null); try { const res = await apiAdmin.extendLoan(loan.loan_id, hours); setNote(`Extended by ${hours}h to ${new Date(res.due_at).toLocaleString()}`); onDone(); } catch (e: any) { setNote(msg(e)); } finally { setBusy(false); } };
