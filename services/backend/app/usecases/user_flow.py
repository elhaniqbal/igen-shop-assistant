...
def parse_slot_steps_60(slot_id: str) -> int:
...
def _ensure_slot_state_seeded(db: Session, tool: models.ToolItem):
    slot_index = parse_slot_steps_60(tool.slot_id)
...
def _current_slot_for_tool(db: Session, tool_item_id: str) -> int:
    ...
    if not row:
        ...
        return parse_slot_steps_60(tool.slot_id)
...
def create_dispense_batch(db: Session, user_id: str, items: list[dict], loan_period_hours: int):
    ...
    for item in items:
        ...
        for _ in range(qty):
            ...
            current_slot = _current_slot_for_tool(db, ti.tool_item_id)
...
def build_hw_payload(db: Session, request_id: str) -> dict:
    ...
    return {
        ...
        "rotation_steps_60": int(lr.slot_id),
        "rotation_direction": "cw" if lr.request_type == "dispense" else "ccw",
