...
class EncoderClient:
    ...
    def _ensure_open(self):
        if self._ser is None:
            try:
                self._ser = serial.Serial(self.port, self.baud, timeout=self.timeout_s, write_timeout=self.timeout_s)
                time.sleep(0.2)
...
class Bridge2:
    ...
    def _run_vertical_home_script(self):
        try:
            proc = subprocess.run(["python3", VERTICAL_HOME_SCRIPT], capture_output=True, text=True, timeout=60, check=False)
        ...
        if proc.returncode != 0:
            raise BridgeError("VERT_HOME_SCRIPT_FAIL", f"Vertical home script failed rc={proc.returncode} stderr={proc.stderr.strip()} stdout={proc.stdout.strip()}")
    ...
    def _rotation_macro(self, direction: str, cake_id: int) -> str:
        return f"MOVE_CAKE_CW_60 CAKE={cake_id}" if direction.upper() == "CW" else f"MOVE_CAKE_CCW_60 CAKE={cake_id}"

    def _execute_rotation(self, request_id: str, cake_id: int, direction: str):
        before = None
        if ENCODER_CONFIRM_ENABLED:
            if not self.encoder:
                raise BridgeError("ENCODER_DISABLED", "Encoder confirmation is enabled but serial encoder client is unavailable")
            before = self.encoder.read_angle(cake_id)
        self._run_gcode(request_id, self._rotation_macro(direction, cake_id))
        if ENCODER_CONFIRM_ENABLED:
            after = self.encoder.read_angle(cake_id)
            expected_delta = 60.0 if direction.upper() == "CW" else -60.0
            actual_delta = normalize_delta_deg(after - before)
            if abs(actual_delta - expected_delta) > ENCODER_TOLERANCE_DEG:
                raise BridgeError("ENCODER_MISMATCH", f"cake={cake_id} expected_delta={expected_delta:.1f} actual_delta={actual_delta:.2f}")

    def _build_dispense_plan(self, payload: dict) -> tuple[int, int, str]:
        return self._require_int(payload, "cake_id"), int(payload.get("rotation_steps_60", 0)), str(payload.get("rotation_direction", "CW")).upper()

    def _build_return_plan(self, payload: dict) -> tuple[int, int, str]:
        return self._require_int(payload, "cake_id"), int(payload.get("rotation_steps_60", 0)), str(payload.get("rotation_direction", "CCW")).upper()
    ...
    def _register_user_confirm(self, request_id: str, action: str, stage: str, timeout_s: float, meta: Optional[dict] = None):
        pending = PendingUserConfirm(request_id=request_id, action=action, stage=stage, created_at=time.monotonic(), timeout_s=timeout_s, meta=meta or {})
    ...
    def _execute_request(self, action: str, payload: dict):
        ...
        try:
            ...
            if action == "dispense":
                cake_id, steps_60, direction = self._build_dispense_plan(payload)
                self._run_gcode(request_id, f"SA_MOVE_TO_CAKE CAKE={cake_id}")
                for _ in range(abs(steps_60)):
                    actual_dir = direction
                    if steps_60 < 0:
                        actual_dir = "CCW" if direction == "CW" else "CW"
                    self._execute_rotation(request_id, cake_id, actual_dir)
                self._run_gcode(request_id, "SA_MOVE_TO_DOOR")
                self._publish_stage(action, request_id, "waiting_user_confirm")
                self._wait_for_user_confirm(request_id, action, "door_take_confirm", DOOR_CONFIRM_TIMEOUT_S, {"message": "Waiting for user to take tool and confirm"})
                self._run_gcode(request_id, "SA_PARK_FROM_DOOR")
            else:
                cake_id, steps_60, direction = self._build_return_plan(payload)
                self._run_gcode(request_id, "SA_MOVE_TO_DOOR")
                self._publish_stage(action, request_id, "waiting_user_confirm")
                self._wait_for_user_confirm(request_id, action, "door_insert_confirm", DOOR_CONFIRM_TIMEOUT_S, {"message": "Waiting for user to insert tool and confirm"})
                self._run_gcode(request_id, f"SA_MOVE_TO_CAKE CAKE={cake_id}")
                for _ in range(abs(steps_60)):
                    actual_dir = direction
                    if steps_60 < 0:
            ...
            self._publish_alert(code=e.code, message=e.reason, severity="critical" if e.code.startswith("KLIPPER") else "error", related_request_id=request_id)
        except Exception as e:
    ...
    def _execute_motor_test(self, payload: dict):
        ...
        try:
            ...
            if action_name == "dispense":
                ...
                self._execute_rotation(request_id, motor_id, "CW")
                self._run_gcode(request_id, "SA_MOVE_TO_DOOR")
                ...
                self._run_gcode(request_id, f"SA_MOVE_TO_CAKE CAKE={motor_id}")
                self._execute_rotation(request_id, motor_id, "CCW")
                self._run_gcode(request_id, "SA_PARK_FROM_CAKE")
            else:
    ...
    def _simulate_motor_test(self, payload: dict):
        ...
        def run():
            ...
            if action_name == "dispense":
                ...
                self._sleep_with_log(request_id, 0.6, "sim rotate cw")
                ...
                self._sleep_with_log(request_id, 5.0, "sim door dwell")
                self._sleep_with_log(request_id, 0.6, "sim move to cake")
                self._sleep_with_log(request_id, 0.6, "sim rotate ccw")
    ...
    def _execute_admin_manual(self, payload: dict):
        ...
        try:
            ...
            if action == "home_machine":
                ...
                direction = str(payload.get("direction", "cw")).upper()
                steps_60 = int(payload.get("steps_60", payload.get("step", 1)))
                for _ in range(abs(steps_60)):
                    actual_dir = direction
                    if steps_60 < 0:
                        actual_dir = "CCW" if direction == "CW" else "CW"
                    self._execute_rotation(rid, cake_id, actual_dir)
                self._publish_alert(code="CAKE_MOVED_MANUALLY", message=f"Cake {cake_id} moved manually", severity="warning", related_request_id=rid, data={"cake_id": cake_id})
